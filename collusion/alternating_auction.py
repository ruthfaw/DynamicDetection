class AlternatingAuction:
# create dual collusion graph
    ...
    def calculate_binding_factor_rating(self, a_rating_array):
        avg_binding_factor = []
        ...
        for x in range(0, self.total_bidder_number):
            ...
            for y in range(0, self.total_bidder_number):
                ...
                while z < addition_limit:
                    ...
                    total_per_group += (min(a_rating_array[index_first], a_rating_array[index_last]) / max(a_rating_array[index_first], a_rating_array[index_last]))
                    z += 1
                    size += 1
            if size == 0:
    ...
    def calculate_collusion_score(self, avg_beta_rating, avg_gamma_rating, avg_delta_rating, avg_epsilon_rating, avg_zeta_rating, avg_binding_factor, collusion_rating):
        ...
        binding_factor_weight = 7

        collusion_score = (((avg_beta_rating * beta_weight) + (avg_gamma_rating * gamma_weight) + (avg_delta_rating * delta_weight) + (avg_epsilon_rating * epsilon_weight) + (avg_zeta_rating * zeta_weight) + (collusion_rating * collusion_rating_weight) + (avg_binding_factor * binding_factor_weight)) / (beta_weight + gamma_weight + delta_weight + epsilon_weight + zeta_weight + collusion_rating_weight + binding_factor_weight)) * 10
